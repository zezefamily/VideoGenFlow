...
import type { AudioTrack, ImageList, ScriptArtifact, StoryboardArtifact, VideoAnalysis, VideoRender } from "@/lib/types";
...
export function WorkflowDock({ convId, script, storyboard, analysis, images, audio, video, isWorking = false, workingLabel, onSend }: Props) {
  ...
  const refresh = () => ["images", "audio-track", "video-render"].forEach((key) => qc.invalidateQueries({ queryKey: [key, convId] }));
  const doTts = async () => {
    setBusy(true);
    try {
  ...
  const doVideo = async () => { setBusy(true); try { await api.renderVideo(convId); refresh(); } finally { setBusy(false); } };
  ...
  if (videoBusy && videoCurrent) { title = "正在合成成片"; detail = video?.stage === "ffmpeg" ? "正在拼接画面、配音与字幕。" : "正在对齐分镜与字幕。"; }
  ...
  else if (video?.status === "done" && videoCurrent) { title = "成片已完成"; detail = `${video.duration_sec?.toFixed(1) || ""} 秒 · 已使用最新配音完成合成。`; primary = "查看成片"; action = () => window.open(api.videoUrl(video.video_url || ""), "_blank"); secondary = "重新合成"; secondaryAction = doVideo; }
  ...
  else if (audio?.status === "done" && audioCurrent && imgDone && !videoCurrent) { title = "新配音已经完成"; detail = "当前成片仍使用旧配音。确认后用新配音重新合成视频。"; primary = "确认，重新合成成片"; action = doVideo; secondary = "重新生成配音"; secondaryAction = doTts; }
  else if (audio?.status === "done" && audioCurrent && imgDone) { title = "素材已经齐备"; detail = "分镜图、配音和字幕均已完成。确认后开始合成 MP4。"; primary = "合成视频"; action = doVideo; secondary = "修改分镜"; }
